...
namespace OWLSharp.Ontology.Rules.BuiltIns
{
    ...
    {
        #region Properties
        internal static readonly RDFResource BuiltInUri = new RDFResource("swrlb:containsIgnoreCase");
        #endregion

        #region Ctors
        internal SWRLContainsIgnoreCaseBuiltIn() { }
        public SWRLContainsIgnoreCaseBuiltIn(SWRLVariableArgument leftArgument, string containString)
            : base(new OWLExpression() { ExpressionIRI = BuiltInUri }, leftArgument, null)
        {
            ...
            BuiltInFilter = new RDFRegexFilter(leftArgument.GetVariable(), new Regex($"{containString}", RegexOptions.IgnoreCase));
        }
        #endregion
    }
}