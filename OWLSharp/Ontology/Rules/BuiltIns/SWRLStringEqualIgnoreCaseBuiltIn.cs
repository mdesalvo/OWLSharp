...
{
    /// <summary>
    /// SWRLStringEqualIgnoreCaseBuiltIn implements the standard http://www.w3.org/2003/11/swrlb#stringEqualIgnoreCase built-in
    /// </summary>
    internal static class SWRLStringEqualIgnoreCaseBuiltIn
    {
        #region Methods
        /// <summary>
        ...
        internal static bool EvaluateOnAntecedent(OWLTableRow antecedentResultsRow, List<SWRLArgument> builtInArguments)
        {
            ...
            switch (builtInArguments[1])
            {
                case SWRLVariableArgument rightArgVar:
                {
                    ...
                    rightPatternMember = RDFQueryUtilities.ParseRDFPatternMember((antecedentResultsRow[rightArgVarName] ?? string.Empty));
                    break;
                }
            ...
            if (isStringLeftPM && isStringRightPM)
            {
                ...
                return string.Equals(leftPMValue, rightPMValue, StringComparison.OrdinalIgnoreCase);
            }
            return false;
        }
        #endregion
