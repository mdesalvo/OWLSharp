...
{
    /// <summary>
    /// SWRLDateTimeBuiltIn implements the standard http://www.w3.org/2003/11/swrlb#dateTime built-in
    /// </summary>
        ...
        internal static bool EvaluateOnAntecedent(RDFTableRow antecedentResultsRow, List<SWRLArgument> builtInArguments)
        {
            ...
            switch (builtInArguments[1])
            {
                case SWRLVariableArgument rightArgVarYEAR:
                {
                    ...
                    rightPatternMemberYEAR = RDFQueryUtilities.ParseRDFPatternMember((antecedentResultsRow[rightArgVarName] ?? string.Empty));
                    break;
                }
            ...
            switch (builtInArguments[2])
            {
                case SWRLVariableArgument rightArgVarMONTH:
                {
                    ...
                    rightPatternMemberMONTH = RDFQueryUtilities.ParseRDFPatternMember((antecedentResultsRow[rightArgVarName] ?? string.Empty));
                    break;
                }
            ...
            switch (builtInArguments[3])
            {
                case SWRLVariableArgument rightArgVarDAY:
                {
                    ...
                    rightPatternMemberDAY = RDFQueryUtilities.ParseRDFPatternMember((antecedentResultsRow[rightArgVarName] ?? string.Empty));
                    break;
                }
