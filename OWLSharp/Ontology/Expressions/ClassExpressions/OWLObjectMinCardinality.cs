...
namespace OWLSharp.Ontology
{
    ...
    public sealed class OWLObjectMinCardinality : OWLClassExpression
    {
        ...
        /// <summary>
        /// The minimum cardinality required on the individuals targeted by the object property expression
        /// </summary>
        ...
        /// <summary>
        /// Builds an OWLObjectMinCardinality with the given object property expression and cardinality
        /// </summary>
        /// <exception cref="OWLException"></exception>
        public OWLObjectMinCardinality(OWLObjectPropertyExpression objectPropertyExpression, uint cardinality)
        {
            ObjectPropertyExpression = objectPropertyExpression ?? throw new OWLException($"Cannot create OWLObjectMinCardinality because given '{nameof(objectPropertyExpression)}' parameter is null");
            Cardinality = cardinality.ToString();
        }

        /// <summary>
        /// Builds an OWLObjectMinCardinality with the given object property expression, cardinality and qualifying class expression
        /// </summary>
        /// <exception cref="OWLException"></exception>
        public OWLObjectMinCardinality(OWLObjectPropertyExpression objectPropertyExpression, uint cardinality, OWLClassExpression classExpression) : this(objectPropertyExpression, cardinality)
            => ClassExpression = classExpression ?? throw new OWLException($"Cannot create OWLObjectMinCardinality because given '{nameof(classExpression)}' parameter is null");
        #endregion

        #region Methods
        /// <summary>
        /// Gets the SWRL representation of this OWLObjectMinCardinality expression
        /// </summary>
        ...
        /// <summary>
        /// Gets the OWL2/Manchester representation of this OWLObjectMinCardinality expression
        /// </summary>
        public override string ToManchesterString(OWLManchesterContext manchesterContext)
            => $"{ObjectPropertyExpression.ToManchesterString(manchesterContext)} min {Cardinality}{(ClassExpression != null ? $" {manchesterContext.Nest(ClassExpression)}" : string.Empty)}";

        /// <summary>
        /// Gets the OWL2/Functional-Style representation of this OWLObjectMinCardinality expression
        /// </summary>
        public override string ToFunctionalString(OWLFunctionalContext functionalContext)
            //The qualifying class expression is optional in the grammar (unqualified cardinality restriction),
            //so it is only appended when present, mirroring the same conditional already used for Manchester
            => $"ObjectMinCardinality( {Cardinality} {ObjectPropertyExpression.ToFunctionalString(functionalContext)}{(ClassExpression != null ? $" {ClassExpression.ToFunctionalString(functionalContext)}" : string.Empty)} )";

        /// <summary>
        /// Exports this OWLObjectMinCardinality expression to an equivalent RDFGraph object
        /// </summary>
