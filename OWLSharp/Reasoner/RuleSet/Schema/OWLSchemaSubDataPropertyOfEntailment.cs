...
{
    internal static class OWLSchemaSubDataPropertyOfEntailment
    {
        internal static readonly string rulename = nameof(OWLEnums.OWLReasonerRules.SchemaSubDataPropertyOfEntailment);

        internal static List<OWLInference> ExecuteRule(OWLOntology ontology)
        {
            List<OWLInference> inferences = new List<OWLInference>();

            //Cheap short-circuit: skip the O(declaredDataProperties) scan below entirely when the R-Box has no SubDataPropertyOf axiom at all
            //(GetSuperDataPropertiesOf would just return empty lists for every declared property in that case)
            List<OWLSubDataPropertyOf> sdpofAxms = ontology.GetDataPropertyAxiomsOfType<OWLSubDataPropertyOf>();
            if (sdpofAxms.Count > 0)
            {
