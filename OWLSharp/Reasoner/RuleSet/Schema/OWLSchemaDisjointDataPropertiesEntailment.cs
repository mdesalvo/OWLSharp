...
{
    internal static class OWLSchemaDisjointDataPropertiesEntailment
    {
        internal static readonly string rulename = nameof(OWLEnums.OWLReasonerRules.SchemaDisjointDataPropertiesEntailment);

        internal static List<OWLInference> ExecuteRule(OWLOntology ontology)
        {
            List<OWLInference> inferences = new List<OWLInference>();

            //Cheap short-circuit: skip the O(declaredDataProperties) scan below entirely when the R-Box has no DisjointDataProperties axiom at all
