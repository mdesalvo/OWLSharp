...
{
    internal static class OWLSchemaDisjointObjectPropertiesEntailment
    {
        internal static readonly string rulename = nameof(OWLEnums.OWLReasonerRules.SchemaDisjointObjectPropertiesEntailment);

        internal static List<OWLInference> ExecuteRule(OWLOntology ontology)
        {
            List<OWLInference> inferences = new List<OWLInference>();

            //Cheap short-circuit: skip the O(declaredObjectProperties) scan below entirely when the R-Box has no DisjointObjectProperties axiom at all
            if (ontology.ObjectPropertyAxioms.Any(opAxm => opAxm is OWLDisjointObjectProperties))
            {
                                                                             ...
                                                                             .Select(ax => (OWLObjectProperty)ax.Entity))
                {
                    ...
                    {
                        //DisjointObjectProperties is symmetric but not auto-normalized: materialize both orderings so
                        //downstream lookups keyed on either property as the "first" operand still find the axiom
                    ...
                    //Object properties are the only R-Box expressions that can appear negated as OWLObjectInverseOf, so disjointness
                    //with an inverse-typed expression needs its own OfType<> branch besides the plain OWLObjectProperty one above
