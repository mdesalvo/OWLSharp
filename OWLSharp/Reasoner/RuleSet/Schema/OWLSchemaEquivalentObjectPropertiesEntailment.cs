...
{
    internal static class OWLSchemaEquivalentObjectPropertiesEntailment
    {
        internal static readonly string rulename = nameof(OWLEnums.OWLReasonerRules.SchemaEquivalentObjectPropertiesEntailment);
            ...
            if (ontology.ObjectPropertyAxioms.Any(opAxm => opAxm is OWLEquivalentObjectProperties))
            {
                                                                             ...
                                                                             .Select(ax => (OWLObjectProperty)ax.Entity))
                {
                    ...
                    if (equivObjectPropertyExprs.Count == 0)
                        continue; //Nothing to propagate for this property: avoid falling through to two no-op OfType<> loops
                    ...
                    //Equivalence with an inverse-typed expression (EquivalentObjectProperties(OP1,ObjectInverseOf(OP2))) needs its own
                    //OfType<> branch, since OWLObjectInverseOf is not an OWLObjectProperty and would be skipped by the loop above
