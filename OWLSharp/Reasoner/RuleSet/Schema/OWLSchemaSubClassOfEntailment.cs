...
{
    internal static class OWLSchemaSubClassOfEntailment
    {
        internal static readonly string rulename = nameof(OWLEnums.OWLReasonerRules.SchemaSubClassOfEntailment);
        ...
        internal static List<OWLInference> ExecuteRule(OWLOntology ontology)
        {
                                                       ...
                                                       .Select(ax => (OWLClass)ax.Entity))
            {
                ...
                foreach (OWLClassExpression superClass in ontology.GetSuperClassesOf(declaredClass))
                {
                    ...
                    //Decompose here (rather than in a dedicated rule) because the intersection is only reachable once
                    //it has already surfaced as a superClass of declaredClass via GetSuperClassesOf just above
