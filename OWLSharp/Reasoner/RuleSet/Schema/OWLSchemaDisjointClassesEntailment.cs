...
{
    internal static class OWLSchemaDisjointClassesEntailment
    {
        internal static readonly string rulename = nameof(OWLEnums.OWLReasonerRules.SchemaDisjointClassesEntailment);

        internal static List<OWLInference> ExecuteRule(OWLOntology ontology)
        {
            List<OWLInference> inferences = new List<OWLInference>();

            //Cheap short-circuit: skip the O(declaredClasses) scan below entirely when the T-Box has no DisjointClasses axiom at all
            ...
            {
                //A single DisjointUnion axiom packs two distinct entailments: (1) all its member classes are pairwise disjoint...
                ...
                //...and (2) each member class is a SubClassOf the union's own named class (the union exhaustively covers C1)
