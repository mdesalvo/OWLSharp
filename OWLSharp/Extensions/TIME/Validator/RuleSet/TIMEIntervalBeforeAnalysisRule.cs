...
{
    internal class TIMEIntervalBeforeAnalysisRule
    {
        internal static readonly string rulename = TIMEEnums.TIMEValidatorRules.IntervalBeforeAnalysis.ToString();
        internal static readonly string rulesugg = "There should not be OWL-TIME intervals having a clash in temporal relations (time:intervalBefore VS {0}";
            ...
            async Task ExecuteRuleBodyAsync(string ruleDescription, string ruleSugg, SWRLObjectPropertyAtom clashingAtom, string shortClashingProperty)
            {
                ...
                violations.ForEach(violation => issues.Add(
                    new OWLIssue(
                        ...
                        rulename,
                        ruleSugg,
                        $"TIME intervals '{((OWLObjectPropertyAssertion)violation.Axiom).SourceIndividualExpression.GetIRI()}' and '{((OWLObjectPropertyAssertion)violation.Axiom).TargetIndividualExpression.GetIRI()}' should be adjusted to not clash on temporal relations (time:intervalBefore VS {shortClashingProperty})"
                    )));
                violations.Clear();
            }
            #endregion
