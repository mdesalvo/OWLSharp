...
{
    internal static class TIMEMeetsStartsEntailmentRule
    {
        internal static async Task<List<OWLInference>> ExecuteRuleAsync(OWLOntology ontology)
        {
            SWRLRule swrlRule = new SWRLRule(
                new RDFPlainLiteral(nameof(TIMEMeetsStartsEntailmentRule)),
                new RDFPlainLiteral("MEETS(?I1,?I2) ^ STARTS(?I2,?I3) -> MEETS(?I1,?I3)"),
                new SWRLAntecedent() 
                {
                    Atoms = new List<SWRLAtom>()
                    {
                        ...
                        new SWRLObjectPropertyAtom(new OWLObjectProperty(RDFVocabulary.TIME.INTERVAL_MEETS), new SWRLVariableArgument(new RDFVariable("?I1")), new SWRLVariableArgument(new RDFVariable("?I2"))),
                        new SWRLObjectPropertyAtom(new OWLObjectProperty(RDFVocabulary.TIME.INTERVAL_STARTS), new SWRLVariableArgument(new RDFVariable("?I2")), new SWRLVariableArgument(new RDFVariable("?I3")))
                    },
                ...
                new SWRLConsequent()
                {
                    Atoms = new List<SWRLAtom>()
                    {
                        new SWRLObjectPropertyAtom(new OWLObjectProperty(RDFVocabulary.TIME.INTERVAL_MEETS), new SWRLVariableArgument(new RDFVariable("?I1")), new SWRLVariableArgument(new RDFVariable("?I3")))
                    }
                });
