...
{
    internal static class TIMEOverlappedByEqualsEntailmentRule
    {
        internal static async Task<List<OWLInference>> ExecuteRuleAsync(OWLOntology ontology)
        {
            SWRLRule swrlRule = new SWRLRule(
                new RDFPlainLiteral(nameof(TIMEOverlappedByEqualsEntailmentRule)),
                new RDFPlainLiteral("OVERLAPPEDBY(?I1,?I2) ^ EQUALS(?I2,?I3) -> OVERLAPPEDBY(?I1,?I3)"),
                new SWRLAntecedent() 
                {
                    Atoms = new List<SWRLAtom>()
                    {
                        ...
                        new SWRLObjectPropertyAtom(new OWLObjectProperty(RDFVocabulary.TIME.INTERVAL_OVERLAPPED_BY), new SWRLVariableArgument(new RDFVariable("?I1")), new SWRLVariableArgument(new RDFVariable("?I2"))),
                ...
                new SWRLConsequent()
                {
                    Atoms = new List<SWRLAtom>()
                    {
                        new SWRLObjectPropertyAtom(new OWLObjectProperty(RDFVocabulary.TIME.INTERVAL_OVERLAPPED_BY), new SWRLVariableArgument(new RDFVariable("?I1")), new SWRLVariableArgument(new RDFVariable("?I3")))
                    }
                });
