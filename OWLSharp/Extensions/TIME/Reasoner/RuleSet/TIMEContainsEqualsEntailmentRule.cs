...
{
    internal static class TIMEContainsEqualsEntailmentRule
    {
        internal static Task<List<OWLInference>> ExecuteRuleAsync(OWLOntology ontology, Dictionary<string, List<OWLIndividualExpression>> cacheRegistry)
        {
            SWRLRule swrlRule = new SWRLRule(
                new RDFPlainLiteral(nameof(TIMEContainsEqualsEntailmentRule)),
                new RDFPlainLiteral("CONTAINS(?I1,?I2) ^ EQUALS(?I2,?I3) -> CONTAINS(?I1,?I3)"),
                new SWRLAntecedent
                {
