...
{
    internal static class TIMEStartsEqualsEntailmentRule
    {
        internal static async Task<List<OWLInference>> ExecuteRuleAsync(OWLOntology ontology)
        {
            SWRLRule swrlRule = new SWRLRule(
                new RDFPlainLiteral(nameof(TIMEStartsEqualsEntailmentRule)),
                new RDFPlainLiteral("STARTS(?I1,?I2) ^ EQUALS(?I2,?I3) -> STARTS(?I1,?I3)"),
                new SWRLAntecedent() 
                {
                ...
                new SWRLConsequent()
                {
                    Atoms = new List<SWRLAtom>()
                    {
                        new SWRLObjectPropertyAtom(new OWLObjectProperty(RDFVocabulary.TIME.INTERVAL_STARTS), new SWRLVariableArgument(new RDFVariable("?I1")), new SWRLVariableArgument(new RDFVariable("?I3")))
                    }
                });
