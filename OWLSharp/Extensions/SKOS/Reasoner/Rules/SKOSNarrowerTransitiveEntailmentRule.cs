...
namespace OWLSharp.Extensions.SKOS
{
    ...
    /// </summary>
    internal static class SKOSNarrowerTransitiveEntailmentRule
    {
        internal static OWLReasonerReport ExecuteRule(OWLOntology ontology)
        {
            SWRLRule swrlRule = new SWRLRule(
                nameof(SKOSNarrowerTransitiveEntailmentRule),
                "NARROWER_TRANSITIVE(?C1,?C2) ^ NARROWER_TRANSITIVE(?C2,?C3) -> NARROWER_TRANSITIVE(?C1,?C3)",
                new SWRLAntecedent()
                    ...
                    .AddAtom(new SWRLObjectPropertyAtom(RDFVocabulary.SKOS.NARROWER_TRANSITIVE, new RDFVariable("?C1"), new RDFVariable("?C2")))
                    .AddAtom(new SWRLObjectPropertyAtom(RDFVocabulary.SKOS.NARROWER_TRANSITIVE, new RDFVariable("?C2"), new RDFVariable("?C3")))
                ...
                new SWRLConsequent()
                    .AddAtom(new SWRLObjectPropertyAtom(RDFVocabulary.SKOS.NARROWER_TRANSITIVE, new RDFVariable("?C1"), new RDFVariable("?C3"))));
