...
{
    /// <summary>
    /// SWRLDifferentFromAtom represents a SWRL atom inferring owl:differentFrom relations between individuals
    /// </summary>
    public class SWRLDifferentFromAtom : SWRLObjectPropertyAtom
    {
        #region Ctors
        /// <summary>
        /// Default-ctor to build an owl:differentFrom atom with the given arguments
        /// </summary>
        public SWRLDifferentFromAtom(RDFVariable leftArgument, RDFVariable rightArgument)
            : base(RDFVocabulary.OWL.DIFFERENT_FROM, leftArgument, rightArgument) { }

        /// <summary>
        /// Default-ctor to build an owl:differentFrom atom with the given arguments
        /// </summary>
        public SWRLDifferentFromAtom(RDFVariable leftArgument, RDFResource rightArgument)
            : base(RDFVocabulary.OWL.DIFFERENT_FROM, leftArgument, rightArgument) { }
        #endregion
                        ...
                        && rightArgumentValue is RDFResource rightArgumentValueResource)
                {
                    ...
                    if (ontology.Data.CheckDifferentFromCompatibility(leftArgumentValueResource, rightArgumentValueResource))
                    {
                        ...
                        if (!ontology.Data.ABoxGraph.ContainsTriple(atomInference))
                            report.AddEvidence(new OWLReasonerEvidence(OWLEnums.OWLReasonerEvidenceCategory.Data, differentFromAtomString, atomInference));
                        if (!ontology.Data.ABoxGraph.ContainsTriple(symmetricAtomInference))
                            report.AddEvidence(new OWLReasonerEvidence(OWLEnums.OWLReasonerEvidenceCategory.Data, differentFromAtomString, symmetricAtomInference));
                    }
                }
            }
