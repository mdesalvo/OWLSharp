...
namespace OWLSharp.Test.Extensions.SWRL.Model.BuiltIns
{
    ...
    public class SWRLSubtractBuiltInTest
    {
        ...
        {
            SWRLSubtractBuiltIn builtin = new SWRLSubtractBuiltIn(new RDFVariable("?L"), new RDFVariable("?R"), 2);

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.GetIRI().Equals(new RDFResource("swrlb:subtract")));
        ...
        public void ShouldThrowExceptionOnCreatingSubtractBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLSubtractBuiltIn(null, new RDFVariable("?R"), 2));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingSubtractBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLSubtractBuiltIn(new RDFVariable("?L"), null, 2));
        ...
        public void ShouldEvaluateSubtractBuiltIn()
        {
            ...
            SWRLSubtractBuiltIn builtin = new SWRLSubtractBuiltIn(new RDFVariable("?X"), new RDFVariable("?Y"), 2);
