...
namespace OWLSharp.Test.Extensions.SWRL.Model.BuiltIns
{
    ...
    public class SWRLTanBuiltInTest
    {
        ...
        {
            SWRLTanBuiltIn builtin = new SWRLTanBuiltIn(new RDFVariable("?L"), new RDFVariable("?R"));

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.GetIRI().Equals(new RDFResource("swrlb:tan")));
        ...
        public void ShouldThrowExceptionOnCreatingTanBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLTanBuiltIn(null, new RDFVariable("?R")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingTanBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLTanBuiltIn(new RDFVariable("?L"), null));
        ...
        public void ShouldEvaluateTanBuiltIn()
        {
            ...
            SWRLTanBuiltIn builtin = new SWRLTanBuiltIn(new RDFVariable("?X"), new RDFVariable("?Y"));
