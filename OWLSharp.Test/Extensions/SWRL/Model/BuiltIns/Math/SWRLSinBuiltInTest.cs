...
namespace OWLSharp.Test.Extensions.SWRL.Model.BuiltIns
{
    ...
    public class SWRLSinBuiltInTest
    {
        ...
        {
            SWRLSinBuiltIn builtin = new SWRLSinBuiltIn(new RDFVariable("?L"), new RDFVariable("?R"));

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.GetIRI().Equals(new RDFResource("swrlb:sin")));
        ...
        public void ShouldThrowExceptionOnCreatingSinBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLSinBuiltIn(null, new RDFVariable("?R")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingSinBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLSinBuiltIn(new RDFVariable("?L"), null));
        ...
        public void ShouldEvaluateSinBuiltIn()
        {
            ...
            SWRLSinBuiltIn builtin = new SWRLSinBuiltIn(new RDFVariable("?X"), new RDFVariable("?Y"));
