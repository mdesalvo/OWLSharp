...
namespace OWLSharp.Test.Extensions.SWRL.Model.BuiltIns
{
    ...
    public class SWRLCosBuiltInTest
    {
        ...
        {
            SWRLCosBuiltIn builtin = new SWRLCosBuiltIn(new RDFVariable("?L"), new RDFVariable("?R"));

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.GetIRI().Equals(new RDFResource("swrlb:cos")));
        ...
        public void ShouldThrowExceptionOnCreatingCosBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLCosBuiltIn(null, new RDFVariable("?R")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingCosBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLCosBuiltIn(new RDFVariable("?L"), null));
        ...
        public void ShouldEvaluateCosBuiltIn()
        {
            ...
            SWRLCosBuiltIn builtin = new SWRLCosBuiltIn(new RDFVariable("?X"), new RDFVariable("?Y"));
