...
namespace OWLSharp.Test.Extensions.SWRL.Model.BuiltIns
{
    ...
    public class SWRLCeilingBuiltInTest
    {
        ...
        {
            SWRLCeilingBuiltIn builtin = new SWRLCeilingBuiltIn(new RDFVariable("?L"), new RDFVariable("?R"));

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.GetIRI().Equals(new RDFResource("swrlb:ceiling")));
        ...
        public void ShouldThrowExceptionOnCreatingCeilingBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLCeilingBuiltIn(null, new RDFVariable("?R")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingCeilingBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLCeilingBuiltIn(new RDFVariable("?L"), null));
        ...
        public void ShouldEvaluateCeilingBuiltIn()
        {
            ...
            SWRLCeilingBuiltIn builtin = new SWRLCeilingBuiltIn(new RDFVariable("?X"), new RDFVariable("?Y"));
