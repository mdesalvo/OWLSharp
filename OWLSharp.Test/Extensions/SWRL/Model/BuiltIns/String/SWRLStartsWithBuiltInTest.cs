...
namespace OWLSharp.Test.Extensions.SWRL.Model.BuiltIns
{
    ...
    public class SWRLStartsWithBuiltInTest
    {
        ...
        {
            SWRLStartsWithBuiltIn builtin = new SWRLStartsWithBuiltIn(new RDFVariable("?L"), "val");

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.BuiltInFilter);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.GetIRI().Equals(new RDFResource("swrlb:startsWith")));
        ...
        public void ShouldThrowExceptionOnCreatingStartsWithBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLStartsWithBuiltIn(null, "val"));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingStartsWithBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new SWRLStartsWithBuiltIn(new RDFVariable("?L"), null));
        ...
        public void ShouldEvaluateStartsWithBuiltIn()
        {
            ...
            SWRLStartsWithBuiltIn builtin = new SWRLStartsWithBuiltIn(new RDFVariable("?C"), "ex1");
