...
{
    [TestClass]
    public class OWLReasonerRuleEndsWithBuiltInTest
    {
        #region Tests
        [TestMethod]
        public void ShouldCreateEndsWithBuiltIn()
        {
            OWLReasonerRuleEndsWithBuiltIn builtin = new OWLReasonerRuleEndsWithBuiltIn(new RDFVariable("?L"), "val");

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.BuiltInFilter);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.Equals(new RDFResource("swrlb:endsWith")));
        ...
        public void ShouldThrowExceptionOnCreatingEndsWithBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleEndsWithBuiltIn(null, "val"));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingEndsWithBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleEndsWithBuiltIn(new RDFVariable("?L"), null));
        ...
        public void ShouldEvaluateEndsWithBuiltIn()
        {
            ...
            OWLReasonerRuleEndsWithBuiltIn builtin = new OWLReasonerRuleEndsWithBuiltIn(new RDFVariable("?C"), "v2");
