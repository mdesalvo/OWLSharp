...
{
    [TestClass]
    public class OWLReasonerRuleAddBuiltInTest
    {
        #region Tests
        [TestMethod]
        public void ShouldCreateAddBuiltIn()
        {
            OWLReasonerRuleAddBuiltIn builtin = new OWLReasonerRuleAddBuiltIn(new RDFVariable("?L"), new RDFVariable("?R"), 2);

            Assert.IsNotNull(builtin);
            Assert.IsNotNull(builtin.Predicate);
            Assert.IsTrue(builtin.Predicate.Equals(new RDFResource("swrlb:add")));
        ...
        public void ShouldThrowExceptionOnCreatingAddBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleAddBuiltIn(null, new RDFVariable("?R"), 2));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingAddBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleAddBuiltIn(new RDFVariable("?L"), null, 2));
        ...
        public void ShouldEvaluateAddBuiltIn()
        {
            ...
            OWLReasonerRuleAddBuiltIn builtin = new OWLReasonerRuleAddBuiltIn(new RDFVariable("?X"), new RDFVariable("?Y"), 2);
