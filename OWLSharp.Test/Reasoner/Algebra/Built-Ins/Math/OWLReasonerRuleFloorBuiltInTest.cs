...
namespace OWLSharp.Reasoner.Test
{
    ...
    public class OWLReasonerRuleFloorBuiltInTest
    {
        ...
        public void ShouldThrowExceptionOnCreatingFloorBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleFloorBuiltIn(null, new RDFVariable("?R")));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingFloorBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleFloorBuiltIn(new RDFVariable("?L"), null));
