...
namespace OWLSharp.Reasoner.Test
{
    ...
    public class OWLReasonerRuleMultiplyBuiltInTest
    {
        ...
        public void ShouldThrowExceptionOnCreatingMultiplyBuiltInBecauseNullLeftArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleMultiplyBuiltIn(null, new RDFVariable("?R"), 2));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingMultiplyBuiltInBecauseNullRightArgument()
            => Assert.ThrowsException<OWLException>(() => new OWLReasonerRuleMultiplyBuiltIn(new RDFVariable("?L"), null, 2));
