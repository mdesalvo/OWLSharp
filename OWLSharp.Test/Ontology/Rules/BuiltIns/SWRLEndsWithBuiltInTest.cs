...
public class SWRLEndsWithBuiltInTest
{
    ...
    public void ShouldCreateEndsWithBuiltIn()
    {
        ...
        Assert.ThrowsExactly<SWRLException>(() => _ = SWRLBuiltIn.EndsWith(null, new SWRLVariableArgument(new RDFVariable("?Y"))));
        Assert.ThrowsExactly<SWRLException>(() => _ = SWRLBuiltIn.EndsWith(new SWRLVariableArgument(new RDFVariable("?X")), null));
    }
    ...
    public void ShouldEvaluateEndsWithBuiltIn()
    {
        ...
        Assert.IsTrue(string.Equals((builtinResults.Rows[1]["?X"] ?? string.Empty), "hello@EN-US"));
        Assert.IsTrue(string.Equals((builtinResults.Rows[1]["?Y"] ?? string.Empty), "lo"));
