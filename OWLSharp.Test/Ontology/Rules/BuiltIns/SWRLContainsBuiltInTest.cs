...
public class SWRLContainsBuiltInTest
{
    ...
    public void ShouldCreateContainsBuiltIn()
    {
        ...
        Assert.ThrowsExactly<SWRLException>(() => _ = SWRLBuiltIn.Contains(null, new SWRLVariableArgument(new RDFVariable("?Y"))));
        Assert.ThrowsExactly<SWRLException>(() => _ = SWRLBuiltIn.Contains(new SWRLVariableArgument(new RDFVariable("?X")), null));
    }
    ...
    [TestMethod]
    public void ShouldEvaluateContainsBuiltInWithIndividualArguments()
    {
        OWLTable antecedentResults = new OWLTable();
