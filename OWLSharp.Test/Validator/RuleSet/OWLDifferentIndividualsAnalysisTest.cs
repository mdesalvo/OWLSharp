...
[TestClass]
public class OWLDifferentIndividualsAnalysisTest
{
    #region Tests
    [TestMethod]
    ...
    public void ShouldAnalyzeDifferentIndividualsSelfCase()
    {
        ...
        Assert.IsTrue(issues.TrueForAll(iss => iss.Severity == OWLEnums.OWLIssueSeverity.Error));
        Assert.IsTrue(issues.TrueForAll(iss => string.Equals(iss.RuleName, OWLDifferentIndividualsAnalysis.rulename)));
        Assert.IsTrue(issues.TrueForAll(iss => string.Equals(iss.Suggestion, OWLDifferentIndividualsAnalysis.rulesugg2)));
    }
    #endregion
}